...
export default function SigninPage() {
  ...
  const onSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // 1. Store token (for persistence)
      localStorage.setItem("accessToken", res.accessToken);

      // 2. Put user + token into Redux global state
      dispatch(
        setCredentials({
      ...
      // 3. Fetch profile using token and hydrate Redux
      ...
      const prev = sessionStorage.getItem("prev_route");
      if (!prev || prev === "/signin" || prev === "/login") {
