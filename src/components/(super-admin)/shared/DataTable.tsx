...
export default function DataTable<T>({
  ...
  return (
    <div className="overflow-hidden border border-gray-300 bg-white/80 backdrop-blur-sm dark:border-gray-700/50 dark:bg-gray-900/30 rounded-xl">
      <div className="flex flex-col items-start sm:flex-row sm:items-center sm:justify-between p-4 sm:p-6">
      ...
      <div className="overflow-hidden px-4 sm:px-6 pb-6">
        <div className="overflow-x-auto rounded-xl border border-gray-200/50 dark:border-gray-700/50 no-scrollbar">
            ...
            <tbody className="divide-y divide-gray-200 bg-white dark:divide-gray-700 dark:bg-gray-900/30">
              ...
              {loading && Array(5).fill(0).map((_, i) => (
                <tr key={`skeleton-row-${i}`}>
                  {visibleColumns.map((col, colIndex) => ( // ✅ FIXED: Use colIndex as key
                    <td key={`skeleton-cell-${i}-${colIndex}`} className="px-6 py-4">
                ...
                <tr>
                  <td colSpan={visibleColumns.length + 1} className="px-6 py-20 text-center">
                    <div className="text-center">
                      ...
                      <h3 className="mb-1 text-lg font-semibold text-gray-900 dark:text-white">No items found</h3>
                      <p className="mb-4 text-gray-500 dark:text-gray-400">Try adjusting your search or filter criteria.</p>
                    </div>
                  </td>
                </tr>
              )}
              ...
              {!loading &&
                data.map((item, i) => (
                  ...
                  >
                    {visibleColumns.map((col) => (
                      <td key={`data-cell-${(item as any)._id || (item as any).id || i}-${col.key}`} className="px-6 py-4 whitespace-nowrap">
        ...
        {!loading && totalPages > 1 && (
          <div className="mt-8 border-t border-gray-200/50 pt-6 dark:border-gray-700/50 px-0 sm:px-0 pb-6">
            <div className="flex flex-col items-start sm:flex-row sm:items-center sm:justify-between">
              <div className="mb-2 text-sm text-gray-700 sm:mb-0 dark:text-gray-300">
                Showing {(page - 1) * limit + 1} to {Math.min(page * limit, totalPlans)} of {totalPlans} results
              </div>
